...
const GroupCreatePage = async ({
  ...
  return (
    ...
    <div className='px-7 flex flex-col'>
      ...
      </p>
      {
        affiliate.status===200 && (
          <div className='w-full mt-5 flex justify-center items-center gap-x-2 italic text-themeTextGray text-sm'> 
            ...
            <Avatar>
              <AvatarImage alt='User' src={affiliate.user?.Group?.User.image as string} />
              <AvatarFallback>
                <User />
              </AvatarFallback>
            </Avatar>
      ...
      <CreateGroup userId={user.id} affiliate={affiliate.status===200?true:false} stripeId={affiliate.user?.Group?.User.stripeId || ""} />
